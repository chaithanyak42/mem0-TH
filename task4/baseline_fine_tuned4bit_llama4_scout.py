"""
Benchmark – fine-tuned 4-bit Llama-3.1-8B (LoRA adapters)   │  Run:
----------------------------------------------------------------┤  python baseline_fine_tuned_4bit_llama31_8b.py
Loads the *base* model in 4-bit (bits-and-bytes NF4), attaches
the LoRA repo  ➜  merges & unloads  ➜  vanilla 4-bit model.
Then times generation for each prompt (same structure as
baseline_inference_ollama.py).

Deps: torch, transformers ≥4.51, peft, bitsandbytes, accelerate, tqdm
"""
...
# ---------------------------------------------------------------------
BASE_MODEL_ID = "unsloth/Llama-4-Scout-17B-16E-Instruct"  # vanilla weights
LORA_REPO_ID  = "cosmos98a/mem0_llama_4_scout_fine_tuned_f16"  # adapters only
...
# ---------------------------------------------------------------------
# 2.  Load model (+ merge LoRA) ------------------------------------------------
# ---------------------------------------------------------------------
def load_model_tokenizer(
    base_id: str = BASE_MODEL_ID, lora_id: str = LORA_REPO_ID
) -> Tuple[AutoModelForCausalLM, AutoTokenizer]:
    """Loads base model in 4-bit, applies LoRA adapters, merges, returns model+tokenizer."""
    logger.info(f"🔄 Loading base model: {base_id} (4-bit NF4)")
    compute_dtype = (
    ...
    # Force single-GPU map to prevent sharding issues after merge
    ...
    logger.info(f"🔄 Attaching LoRA repo: {lora_id}")
    # Also specify device_map here for consistency, although base model dictates it
    peft_model = PeftModel.from_pretrained(base, lora_id, device_map={"": 0})

    logger.info("🔄 Merging LoRA into 4-bit weights …")
    t0 = time.time()
    model = peft_model.merge_and_unload()  # Returns plain model, already on cuda:0
    logger.info(f"✅ merge done in {time.time() - t0:.1f}s")

    tokenizer = AutoTokenizer.from_pretrained(lora_id, use_fast=True)
...
async def bench() -> Dict:
    ...
    summary = {
        "model": f"{BASE_MODEL_ID}+LoRA[{LORA_REPO_ID}]",
    ...
    # save
    safe_name = LORA_REPO_ID.replace("/", "_")
