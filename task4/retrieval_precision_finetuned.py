...
MODEL_TAG  = "cosmos98a/mem0-merged-llama3.1-8b-4bit" 
EMBED_TAG  = "nomic-embed-text"                  
USER_ID    = "debug-user-finetuned" 
AGENT_ID   = "debug-agent-finetuned" 
TOP_K      = 5
...
if MEMORIES and QUERIES:
    assert len(MEMORIES) == len(QUERIES) == 100
else:
    print("WARNING: MEMORIES and/or QUERIES lists are empty. Paste them from the original script.")
    
def ensure_ollama_embedder(tag: str) -> None:
    """Ensures the Ollama embedder model is present."""
    if shutil.which("ollama") is None:
        sys.exit("Install Ollama first: https://ollama.com (needed for embedder)")
    models = subprocess.run(["ollama", "list"], capture_output=True, text=True).stdout
    if tag.split(":")[0] not in models:
        print(f"Pulling Ollama embedder {tag} …", flush=True)
        subprocess.run(["ollama", "pull", tag], check=True)


async def run_benchmark() -> Dict:
    ensure_ollama_embedder(EMBED_TAG)

   
    qdrant_data_path = os.path.join(mem0_dir, f"qdrant_data_{USER_ID}")
    history_db_path = os.path.join(mem0_dir, f"history_{USER_ID}.db")

    if os.path.exists(qdrant_data_path):
        print(f"Cleaning up old vector store data for {USER_ID}...")
        try:
    ...
    print("\nSetting up with embedding dims:", EMBEDDING_DIMS)
    ...
    cfg.llm = LlmConfig(
        provider="llama31_8b_finetuned_4bit",
        config={
            "model_id": MODEL_TAG,
        }
    )
    ...
    print(f"  LLM model_id: {cfg.llm.config.get('model_id')}")
    ...
    try:
        print("Initializing AsyncMemory with fine-tuned LLM...")
        mem = AsyncMemory(cfg)
        print("AsyncMemory initialized successfully.")
    except Exception as e:
        print(f"Error initializing AsyncMemory: {e}")
        print("Check if the custom LLM provider ('llama31_8b_finetuned') is correctly registered in mem0/utils/factory.py")
        sys.exit(1)
    ...
    try:
        ...
        if len(test_vector) != EMBEDDING_DIMS:
            print(f"CRITICAL ERROR: Actual embedding dimensions ({len(test_vector)}) don't match configured dimensions ({EMBEDDING_DIMS})")
            sys.exit("Please fix EMBEDDING_DIMS constant.")

    except ImportError:
        print("Skipping embedder dimension test (OllamaEmbedding not found?)")
    except Exception as e:
        print(f"Error during embedder dimension test: {e}")


  
    if not MEMORIES or not QUERIES:
        sys.exit("MEMORIES and QUERIES lists are empty. Cannot proceed.")

    print("Adding memories using fine-tuned LLM for processing...")
    id_map: Dict[str, str] = {}
    for mem_text in tqdm(MEMORIES, unit="mem", desc="Adding Memories"):
        try:
            res = await mem.add(mem_text, user_id=USER_ID, agent_id=AGENT_ID)
            if res and "results" in res and res["results"] and "id" in res["results"][0]:
                 id_map[mem_text] = res["results"][0]["id"]
            else:
                print(f"    Unexpected result format from mem.add for text '{mem_text[:50]}...': {res}")
                id_map[mem_text] = None # Mark as failed/missing ID
        except Exception as e:
            print(f"Error during mem.add for text '{mem_text[:50]}...': {e}")
            id_map[mem_text] = None

    print("\nSearching memories...")
    hits, rows = 0, []
    for mem_txt, q in atqdm(list(zip(MEMORIES, QUERIES)), total=len(QUERIES), unit="qry", desc="Searching"):
        target_id = id_map.get(mem_txt) 

        if target_id is None:
            print(f"\nSkipping query for '{mem_txt[:50]}...' as its memory ID is missing.")
            rows.append(
                {"query": q, "target": mem_txt, "target_id": "MISSING",
                 "hit": 0, "topK_ids": "[]", "top1_memory": "SKIPPED"}
            )
            continue

        try:
            res   = await mem.search(q, user_id=USER_ID, agent_id=AGENT_ID, limit=TOP_K)
            topKs = [r["id"] for r in res.get("results", [])] if res else []
            hit   = target_id in topKs
            hits += int(hit)
            rows.append(
                {"query": q, "target": mem_txt, "target_id": target_id,
                 "hit": int(hit), "topK_ids": json.dumps(topKs),
                 "top1_memory": res["results"][0]["memory"] if res and res.get("results") else ""}
            )
        except Exception as e:
            print(f"Error during mem.search for query '{q[:50]}...': {e}")
            rows.append(
                 {"query": q, "target": mem_txt, "target_id": target_id,
                  "hit": 0, "topK_ids": "ERROR", "top1_memory": f"ERROR: {e}"}
            )


    num_processed_queries = sum(1 for r in rows if r["target_id"] != "MISSING" and r["topK_ids"] != "ERROR")
    p_at_5 = (hits / num_processed_queries) if num_processed_queries > 0 else 0.0
    ...
    raw_csv = OUT_DIR / "finetuned_retrieval_raw.csv"
    summary_json = OUT_DIR / "finetuned_retrieval_summary.json"
    ...
    if rows: 
        ...
        print("\nNo rows generated, skipping raw CSV save.")
...
if __name__ == "__main__":
    print(f"Retrieval benchmark for FINE-TUNED LLM: {MODEL_TAG}")
    print(f"      with embedder: {EMBED_TAG}")
    if not MEMORIES or not QUERIES:
         print("\nERROR: MEMORIES and/or QUERIES lists are empty in the script.")
         print("      Please paste them from retrieval_precision_ollama.py before running.")
         sys.exit(1)

    summary = asyncio.run(run_benchmark())
    print("\n=== Fine-tuned Retrieval Summary ===")
